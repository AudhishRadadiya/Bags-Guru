...
export default function DuplicateBag() {
  ...
  const {
    bagLoading,
    bagCRUDLoading,
    isGetInitialValuesForDuplicateBag,
  ...
  const duplicateBagInitialValues = {
  ...
  const [initialValuesForDuplicateBag, setInitialValuesForDuplicateBag] =
    useState(duplicateBagInitialValues);
  ...
  useEffect(() => {
    if (!isGetInitialValuesForDuplicateBag && bag_id) {
      dispatch(getBagItem(bag_id, 'isDuplicated'));
    }
  }, [bag_id]);

  useEffect(() => {
    if (isGetInitialValuesForDuplicateBag) {
      if (Object.keys(selectedBagForDuplicate)?.length > 0) {
        setInitialValuesForDuplicateBag(selectedBagForDuplicate);
      } else {
        setInitialValuesForDuplicateBag(duplicateBagInitialValues);
      }
    } else {
      if (Object.keys(bagDetailForDuplicate)?.length > 0) {
        setInitialValuesForDuplicateBag(bagDetailForDuplicate);
        dispatch(setSelectedBagForDuplicate(bagDetailForDuplicate));
        dispatch(setIsGetInitialValuesForDuplicateBag(true));
      }
    }
  }, [bagDetailForDuplicate]);
  ...
  const {
    ...
    initialValues: initialValuesForDuplicateBag,
  ...
  const commonUpdateFiledValue = useCallback(
    ...
    [dispatch, handleChange, selectedBagForDuplicate],
  );
  const commonUpdateFiledSetValues = useCallback(
    ...
    [dispatch, setFieldValue, selectedBagForDuplicate],
  );
  ...
  }, [values?.bag_type, activeBagTypeList]);
  ...
  useMemo(() => {
    ...
    commonUpdateFiledSetValues('bag_weight', `${val ? val?.toFixed(2) : 0} gm`);
  }, [
    suggestedRL,
    suggestedRLInMM,
    suggestedRW,
                ...
                <Col md={5} xs={12}>
                  <div className="form_group mb-3">
                    ...
                    <ReactSelectSingle
                      ...
                      onChange={e => {
                        commonUpdateFiledValue('material', e.target.value);
                      }}
                      onBlur={handleBlur}
                ...
                <Col md={5}>
                  <div className="form_group mb-3">
                    ...
                    <ReactSelectSingle
                      ...
                      onChange={e => {
                        commonUpdateFiledValue('form', e.target.value);
                      }}
                      onBlur={handleBlur}
                          ...
                          <Col lg={4} md={6} key={i}>
                            <div className="form_group">
                              <input
                                ...
                                disabled={values?.is_laminated === 0}
                              />
                      ...
                      <Checkbox
                        ...
                        onChange={e => {
                          ...
                          dispatch(
                            setSelectedBagForDuplicate({
                              ...selectedBagForDuplicate,
                              handle_weight_auto: e.checked ? 0 : 1,
                        ...
                        onBlur={handleBlur}
                        checked={values?.handle_weight_auto === 0}
                      />
                      <label className="ms-2">
                        <div className="form_group">
                          ...
                          <InputText
                            ...
                            disabled={values?.handle_weight_auto === 1}
                            onBlur={handleBlur}
                          />
