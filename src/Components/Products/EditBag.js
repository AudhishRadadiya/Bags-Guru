...
import { getBagItem, updateBag } from 'Services/Products/BagService';
import {
  setBagDetailForEdit,
...
export default function EditBag() {
  ...
  const [initialValuesForEditBag, setInitialValuesForEditBag] =
    useState(editBagInitialValues);
  ...
  useEffect(() => {
    if (!isGetInitialValuesForEditBag && bag_id) {
      dispatch(getBagItem(bag_id, 'isEdit'));
    }
  }, [bag_id]);

  useEffect(() => {
    if (isGetInitialValuesForEditBag) {
      if (Object.keys(selectedBagForEdit)?.length > 0) {
        setInitialValuesForEditBag(selectedBagForEdit);
      } else {
        setInitialValuesForEditBag(editBagInitialValues);
      }
    } else {
      if (Object.keys(bagDetailForEdit)?.length > 0) {
        setInitialValuesForEditBag(bagDetailForEdit);
        dispatch(setSelectedBagForEdit(bagDetailForEdit));
        dispatch(setIsGetInitialValuesForEditBag(true));
      }
    }
  }, [bagDetailForEdit]);
    ...
    async values => {
      ...
      if (values?._id) {
        const payload = {
          ...
          handle_weight_auto: values?.handle_weight_auto ? 1 : 0,
          is_active: values?.is_active ? 1 : 0,
        };
        result = await dispatch(updateBag(payload));
      }
      if (result) {
        navigate('/bags');
        setInitialValuesForEditBag(editBagInitialValues);
  ...
  const {
    ...
    initialValues: initialValuesForEditBag,
  ...
  const commonUpdateFiledValue = useCallback(
    ...
    [dispatch, handleChange, selectedBagForEdit],
  );
  const commonUpdateFiledSetValues = useCallback(
    ...
    [dispatch, setFieldValue, selectedBagForEdit],
  );
  ...
  useEffect(() => {
    ...
    //     'handle_weight',
    //     activeBagTypeList?.find(x => x?._id === values?.bag_type)
    //       ?.handle_weight,
    //   );
    commonUpdateFiledSetValues(
      'selected_bag_type_name',
                      ...
                      <label className="ms-2">
                        <div className="form_group">
                          ...
                          <InputText
                            ...
                            disabled={values?.handle_weight_auto === 0}
                            onBlur={handleBlur}
                          />
