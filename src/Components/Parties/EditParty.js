...
const EditParty = () => {
  ...
  const { isGetInitialValuesParty, updateSelectedPartyData } = useSelector(
    ({ parties }) => parties,
  );
  ...
  useEffect(() => {
    if (id) {
      if (isGetInitialValuesParty?.update === true) {
        setInitialData(updateSelectedPartyData);
      } else {
        handlePartiesItem();
      }
    }
