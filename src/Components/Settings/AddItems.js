...
import { getUniqItems, rawItemAttributeType } from 'Helper/Common';
...
export default function AddItems() {
  ...
  const handleChangeColors = useCallback(
    (key, val) => {
      if (key === 'color') setColorCode(val);
      else if (key === 'color_name') setColorName(val);
      else if (key === 'remove') {
  ...
  const onCancel = useCallback(() => {
    ...
    dispatch(clearSelectedRawItem());
    navigate('/items');
  }, [attributeFormik, dispatch, navigate, resetForm]);
              ...
              <Col md={12}>
                <div className="form_group checkbox_wrap">
                  ...
                  <Checkbox
                    ...
                    disabled={state?.isView}
                  />
                  <label htmlFor="RegularCustomer" className="mb-0">
