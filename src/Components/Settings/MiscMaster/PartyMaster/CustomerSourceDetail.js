...
import {
  createCustomerSourceDetail,
  deleteCustomerSourceDetail,
  getCustomerSourceDetailList,
  updateCustomerSourceDetail,
} from 'Services/Settings/MiscMasterService';
import { Tag } from 'primereact/tag';
import { useFormik } from 'formik';
import { addCustomerSourceDetailSchema } from 'Schemas/Settings/MiscMasterSchema';
import { setSelectedCustomerSourceDetail } from 'Store/Reducers/Settings/MiscMasterSlice';
...
export default function CustomerSourceDetail({ hasAccess }) {
        ...
        dispatch(
          setAllFilters({
            ...
            customerSourceDetail: {
              ...allFilters?.customerSourceDetail,
              currentPage: 1,
            },
          }),
        );
  ...
  const {
    ...
    initialValues: selectedCustomerSourceDetail,
    validationSchema: addCustomerSourceDetailSchema,
  ...
  const handleEdit = useCallback(
    customerSourceDetail_id => {
      const customerSourceDetail = customerSourceDetailList?.find(
        x => x?._id === customerSourceDetail_id,
      );
      if (customerSourceDetail) {
        dispatch(setSelectedCustomerSourceDetail(customerSourceDetail));
        setSaveFilterModal(true);
      }
    },
    [dispatch, customerSourceDetailList],
  );

  const handleDelete = useCallback(
    async customerSourceDetail_id => {
  ...
  const handleSearchInput = e => {
    ...
    dispatch(getCustomerSourceDetailList(pageLimit, 1, e.target.value));
  };
    ...
    <>
      ...
      <div className="table_main_Wrapper bg-white">
          ...
          <Row className="align-items-center">
            <Col md={3}>
              <div className="page_title">
                <h3 className="m-0">Customer Source Detail</h3>
              </div>
            </Col>
            <Col md={9}>
              <div className="right_filter_wrapper">
                <ul>
                  ...
                  <li>
                    ...
                    >
                      <img src={PlusIcon} alt="" /> Add Customer Source Detail
                    </Button>
                  </li>
                </ul>
              </div>
        ...
        <div className="data_table_wrapper misc_master_table_wrapper with_colspan_head cell_padding_large is_filter">
          ...
          <DataTable
            value={customerSourceDetailList}
            sortMode="multiple"
            sortField="name"
            sortOrder={1}
            ...
            filterDisplay="row"
            dataKey="_id"
            filters={customerSourceDetailFilters}
            onFilter={event => {
              dispatch(
                setAllCommon({
                  ...allCommon,
                  customerSourceDetail: {
                    ...allCommon?.customerSourceDetail,
                    customerSourceDetailFilters: event?.filters,
                  },
                }),
              );
            }}
          ...
          <CustomPaginator
            dataList={customerSourceDetailList}
            pageLimit={pageLimit}
            onPageChange={onPageChange}
            onPageRowsChange={onPageRowsChange}
            currentPage={currentPage}
            totalCount={customerSourceDetailCount}
          />
        </div>
        <Dialog
          header={`${
            selectedCustomerSourceDetail?._id ? 'Edit' : 'Add'
        ...
        >
          ...
          <div className="button_group d-flex align-items-center justify-content-end pt-3">
            ...
            >
              {selectedCustomerSourceDetail?._id ? 'Update' : 'Save'}
            </Button>
          </div>
        </Dialog>
      </div>
