...
import { addPartyTypeSchema } from 'Schemas/Settings/MiscMasterSchema';
import { setSelectedPartyType } from 'Store/Reducers/Settings/MiscMasterSlice';
...
export default function PartyType({ hasAccess }) {
  ...
  const { searchQuery, partyTypeFilters, filterToggle } = allCommon?.partyType;
  const { currentPage, pageLimit } = allFilters?.partyType;

  useEffect(() => {
    const getData = setTimeout(() => {
      dispatch(getPartyTypeList(pageLimit, currentPage, searchQuery));
    }, 700);
  ...
  const submitHandle = useCallback(
    async values => {
      let result;
      if (selectedPartyType?._id) {
        const payload = {
          ...values,
          partyType_id: selectedPartyType?._id,
        };
        result = await dispatch(updatePartyType(payload));
      } else {
        const payload = {
          ...values,
        };
        result = await dispatch(createPartyType(payload));
      }
      if (result) {
        resetForm();
        dispatch(
          setAllFilters({
            ...allFilters,
            partyType: {
              ...allFilters?.partyType,
              currentPage: 1,
            },
          }),
        );
        setSaveFilterModal(false);
        dispatch(setSelectedPartyType({ name: '', is_active: 1 }));
        dispatch(getPartyTypeList(pageLimit, 1, searchQuery));
      }
    },
    [dispatch, pageLimit, searchQuery, selectedPartyType?._id, allFilters],
  );
  ...
  const {
    ...
    initialValues: selectedPartyType,
    validationSchema: addPartyTypeSchema,
  ...
  const handleEdit = useCallback(
    partyType_id => {
      const partyType = partyTypeList?.find(x => x?._id === partyType_id);
      if (partyType) {
        dispatch(setSelectedPartyType(partyType));
        setSaveFilterModal(true);
      }
    },
    [dispatch, partyTypeList],
  );

  const handleDelete = useCallback(
    async partyType_id => {
      if (partyType_id) {
        const result = await dispatch(deletePartyType(partyType_id));
        if (result) {
  ...
  const handleSearchInput = e => {
    ...
    dispatch(getPartyTypeList(pageLimit, currentPage, e.target.value));
  };
  ...
  return (
    <>
      ...
      <div className="table_main_Wrapper bg-white">
          ...
          <Row className="align-items-center">
            <Col md={3}>
              <div className="page_title">
                <h3 className="m-0">Party Type</h3>
              </div>
            </Col>
            <Col md={9}>
              <div className="right_filter_wrapper">
                <ul>
                  ...
                  <li>
                    ...
                    >
                      <img src={PlusIcon} alt="" /> Add Party Type
                    </Button>
                  </li>
                </ul>
              </div>
            ...
            onFilter={event => {
              dispatch(
                setAllCommon({
                  ...
                  partyType: {
                    ...allCommon?.partyType,
                    partyTypeFilters: event?.filters,
                  },
                }),
              );
            }}
        ...
        <Dialog
          header={`${selectedPartyType?._id ? 'Edit' : 'Add'} Party Type`}
        ...
        >
          ...
          <div className="button_group d-flex align-items-center justify-content-end pt-3">
            ...
            >
              {selectedPartyType?._id ? 'Update' : 'Save'}
            </Button>
          </div>
        </Dialog>
      </div>
