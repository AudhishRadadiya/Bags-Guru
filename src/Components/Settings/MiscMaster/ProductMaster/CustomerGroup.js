...
import { addCustomerGroupSchema } from 'Schemas/Settings/MiscMasterSchema';
import { setSelectedCustomerGroup } from 'Store/Reducers/Settings/MiscMasterSlice';
...
export default function CustomerGroup({ hasAccess }) {
  ...
  const { searchQuery, customerGroupFilters, filterToggle } =
    allCommon?.customerGroup;
  const { currentPage, pageLimit } = allFilters?.customerGroup;

  useEffect(() => {
    const getData = setTimeout(() => {
      dispatch(getCustomerGroupList(pageLimit, currentPage, searchQuery));
    }, 700);
  ...
  const submitHandle = useCallback(
    async values => {
      let result;
      if (selectedCustomerGroup?._id) {
        const payload = {
          ...values,
          customer_group_id: selectedCustomerGroup?._id,
        };
        result = await dispatch(updateCustomerGroup(payload));
      } else {
        const payload = {
          ...values,
        };
        result = await dispatch(createCustomerGroup(payload));
      }
      if (result) {
        resetForm();
        dispatch(
          setAllFilters({
            ...allFilters,
            customerGroup: {
              ...allFilters?.customerGroup,
              currentPage: 1,
            },
          }),
        );
        setSaveFilterModal(false);
        dispatch(setSelectedCustomerGroup({ name: '', is_active: 1 }));
        dispatch(getCustomerGroupList(pageLimit, 1, searchQuery));
      }
    },
    [dispatch, pageLimit, searchQuery, selectedCustomerGroup?._id, allFilters],
  );
  ...
  const {
    ...
    validationSchema: addCustomerGroupSchema,
  ...
  const handleEdit = useCallback(
    customer_group_id => {
      const customerGroup = customerGroupList?.find(
        x => x?._id === customer_group_id,
      );
      if (customerGroup) {
        dispatch(setSelectedCustomerGroup(customerGroup));
        setSaveFilterModal(true);
      }
    },
    [dispatch, customerGroupList],
  );

  const handleDelete = useCallback(
    async customer_group_id => {
      if (customer_group_id) {
        const result = await dispatch(deleteCustomerGroup(customer_group_id));
        if (result) {
  ...
  const handleSearchInput = e => {
    ...
    dispatch(getCustomerGroupList(pageLimit, currentPage, e.target.value));
  };
    ...
    <>
      ...
      <div className="table_main_Wrapper bg-white">
          ...
          <Row className="align-items-center">
            <Col md={3}>
              <div className="page_title">
                <h3 className="m-0">Customer Group</h3>
              </div>
            </Col>
            <Col md={9}>
              <div className="right_filter_wrapper">
                <ul>
                  ...
                  <li>
                    ...
                    >
                      <img src={PlusIcon} alt="" /> Add Customer Group
                    </Button>
                  </li>
                </ul>
              </div>
        ...
        <div className="data_table_wrapper misc_master_table_wrapper with_colspan_head cell_padding_large is_filter">
            ...
            onFilter={event => {
              dispatch(
                setAllCommon({
                  ...
                  customerGroup: {
                    ...allCommon?.customerGroup,
                    customerGroupFilters: event?.filters,
                  },
                }),
              );
            }}
          ...
          <CustomPaginator
            dataList={customerGroupList}
            pageLimit={pageLimit}
            onPageChange={onPageChange}
            onPageRowsChange={onPageRowsChange}
            currentPage={currentPage}
            totalCount={customerGroupCount}
          />
        </div>
        <Dialog
          header={`${
            selectedCustomerGroup?._id ? 'Edit' : 'Add'
          } Customer Group`}
        ...
        >
          ...
          <div className="button_group d-flex align-items-center justify-content-end pt-3">
            ...
            >
              {selectedCustomerGroup?._id ? 'Update' : 'Save'}
            </Button>
          </div>
        </Dialog>
      </div>
