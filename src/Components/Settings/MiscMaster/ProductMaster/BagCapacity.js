...
import { addBagCapacitySchema } from 'Schemas/Settings/MiscMasterSchema';
import { setSelectedBagCapacity } from 'Store/Reducers/Settings/MiscMasterSlice';
...
export default function BagCapacity({ hasAccess }) {
  ...
  const { searchQuery, bagCapacityFilters, filterToggle } =
    allCommon?.bagCapacity;
  const { currentPage, pageLimit } = allFilters?.bagCapacity;

  useEffect(() => {
    const getData = setTimeout(() => {
      dispatch(getBagCapacityList(pageLimit, currentPage, searchQuery));
    }, 700);
  ...
  const submitHandle = useCallback(
    async values => {
      let result;
      if (selectedBagCapacity?._id) {
        const payload = {
          ...values,
          bagCapacity_id: selectedBagCapacity?._id,
        };
        result = await dispatch(updateBagCapacity(payload));
      } else {
        const payload = {
          ...values,
        };
        result = await dispatch(createBagCapacity(payload));
      }
      if (result) {
        ...
        dispatch(
          setAllFilters({
            ...allFilters,
            bagCapacity: {
              ...allFilters?.bagCapacity,
              currentPage: 1,
            },
          }),
        );
        dispatch(setSelectedBagCapacity({ size: '', is_active: 1 }));
        dispatch(getBagCapacityList(pageLimit, 1, searchQuery));
      }
    },
    [dispatch, pageLimit, searchQuery, selectedBagCapacity?._id, allFilters],
  );
  ...
  const {
    ...
    initialValues: selectedBagCapacity,
    validationSchema: addBagCapacitySchema,
  ...
  const handleEdit = useCallback(
    bagCapacity_id => {
      const bagCapacity = bagCapacityList?.find(x => x?._id === bagCapacity_id);
      if (bagCapacity) {
        dispatch(setSelectedBagCapacity(bagCapacity));
        setSaveFilterModal(true);
      }
    },
    [dispatch, bagCapacityList],
  );

  const handleDelete = useCallback(
    async bagCapacity_id => {
      if (bagCapacity_id) {
        const result = await dispatch(deleteBagCapacity(bagCapacity_id));
        if (result) {
  ...
  const handleSearchInput = e => {
    ...
    dispatch(getBagCapacityList(pageLimit, currentPage, e.target.value));
  };
    ...
    <>
      ...
      <div className="table_main_Wrapper bg-white">
          ...
          <Row className="align-items-center">
            <Col md={3}>
              <div className="page_title">
                <h3 className="m-0">Bag Capacity</h3>
              </div>
            </Col>
            <Col md={9}>
              <div className="right_filter_wrapper">
                <ul>
                  ...
                  <li>
                    ...
                    >
                      <img src={PlusIcon} alt="" /> Add Bag Capacity
                    </Button>
                  </li>
                </ul>
              </div>
        ...
        <div className="data_table_wrapper misc_master_table_wrapper  with_colspan_head cell_padding_large is_filter">
            ...
            onFilter={event => {
              dispatch(
                setAllCommon({
                  ...
                  bagCapacity: {
                    ...allCommon?.bagCapacity,
                    bagCapacityFilters: event?.filters,
                  },
                }),
              );
            }}
          ...
          <CustomPaginator
            dataList={bagCapacityList}
            pageLimit={pageLimit}
            onPageChange={onPageChange}
            onPageRowsChange={onPageRowsChange}
            currentPage={currentPage}
            totalCount={bagCapacityCount}
          />
        </div>
        <Dialog
          header={`${selectedBagCapacity?._id ? 'Edit' : 'Add'} Bag Capacity`}
        ...
        >
          ...
          <div className="button_group d-flex align-items-center justify-content-end pt-3">
            ...
            >
              {selectedBagCapacity?._id ? 'Update' : 'Save'}
            </Button>
          </div>
        </Dialog>
      </div>
