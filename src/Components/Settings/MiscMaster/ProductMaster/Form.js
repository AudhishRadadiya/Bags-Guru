...
import { addFormSchema } from 'Schemas/Settings/MiscMasterSchema';
import { setSelectedForm } from 'Store/Reducers/Settings/MiscMasterSlice';
...
export default function Form({ hasAccess }) {
  ...
  const { searchQuery, formFilters, filterToggle } = allCommon?.form;
  const { currentPage, pageLimit } = allFilters?.form;

  useEffect(() => {
    const getData = setTimeout(() => {
      dispatch(getFormList(pageLimit, currentPage, searchQuery));
    }, 700);
  ...
  const submitHandle = useCallback(
    async values => {
      ...
      if (selectedForm?._id) {
        ...
        result = await dispatch(updateForm(payload));
      } else {
        const payload = {
          ...values,
        };
        result = await dispatch(createForm(payload));
      }
      if (result) {
        resetForm();
        dispatch(
        ...
        dispatch(setSelectedForm({ name: '', is_active: 1 }));
        dispatch(getFormList(pageLimit, 1, searchQuery));
      }
    },
    [dispatch, pageLimit, searchQuery, selectedForm?._id, allFilters],
  );
  ...
  const handleEdit = useCallback(
    form_id => {
      const form = formList?.find(x => x?._id === form_id);
      if (form) {
  ...
  const handleDelete = useCallback(
    async form_id => {
      if (form_id) {
        const result = await dispatch(deleteForm(form_id));
        if (result) {
  ...
  const handleSearchInput = e => {
    ...
    dispatch(getFormList(pageLimit, currentPage, e.target.value));
  };
  ...
  return (
    <>
      ...
      <div className="table_main_Wrapper bg-white">
        ...
        <Dialog
          header={`${selectedForm?._id ? 'Edit' : 'Add'} Form`}
