...
import { addBagMachineSchema } from 'Schemas/Settings/MiscMasterSchema';
import { setSelectedBagMachine } from 'Store/Reducers/Settings/MiscMasterSlice';
...
export default function BagMachine({ hasAccess }) {
  ...
  const { searchQuery, bagMachineFilters, filterToggle } =
    allCommon?.bagMachine;
  const { currentPage, pageLimit } = allFilters?.bagMachine;

  useEffect(() => {
    const getData = setTimeout(() => {
      dispatch(getBagMachineList(pageLimit, currentPage, searchQuery));
    }, 700);
  ...
  const submitHandle = useCallback(
    async values => {
      let result;
      if (selectedBagMachine?._id) {
        const payload = {
          ...values,
          bagMachine_id: selectedBagMachine?._id,
        };
        result = await dispatch(updateBagMachine(payload));
      } else {
        const payload = {
          ...values,
        };
        result = await dispatch(createBagMachine(payload));
      }
      if (result) {
        resetForm();
        dispatch(
          setAllFilters({
            ...allFilters,
            bagMachine: {
              ...allFilters?.bagMachine,
              currentPage: 1,
            },
          }),
        );
        setSaveFilterModal(false);
        dispatch(setSelectedBagMachine({ name: '', code: '', is_active: 1 }));
        dispatch(getBagMachineList(pageLimit, 1, searchQuery));
      }
    },
    [dispatch, pageLimit, searchQuery, selectedBagMachine?._id, allFilters],
  );
  ...
  const {
    ...
    initialValues: selectedBagMachine,
    validationSchema: addBagMachineSchema,
  ...
  const handleEdit = useCallback(
    bagMachine_id => {
      const bagMachine = bagMachineList?.find(x => x?._id === bagMachine_id);
      if (bagMachine) {
        dispatch(setSelectedBagMachine(bagMachine));
        setSaveFilterModal(true);
      }
    },
    [dispatch, bagMachineList],
  );

  const handleDelete = useCallback(
    async bagMachine_id => {
      if (bagMachine_id) {
        const result = await dispatch(deleteBagMachine(bagMachine_id));
        if (result) {
  ...
  const handleSearchInput = e => {
    ...
    dispatch(getBagMachineList(pageLimit, currentPage, e.target.value));
  };
    ...
    <>
      ...
      <div className="table_main_Wrapper bg-white">
          ...
          <Row className="align-items-center">
            <Col md={3}>
              <div className="page_title">
                <h3 className="m-0">Bag Machine</h3>
              </div>
            </Col>
            <Col md={9}>
              <div className="right_filter_wrapper">
                <ul>
                  ...
                  <li>
                    ...
                    >
                      <img src={PlusIcon} alt="" /> Add Bag Machine
                    </Button>
                  </li>
                </ul>
              </div>
        ...
        <div className="data_table_wrapper misc_master_table_wrapper with_colspan_head cell_padding_large is_filter">
            ...
            onFilter={event => {
              dispatch(
                setAllCommon({
                  ...
                  bagMachine: {
                    ...allCommon?.bagMachine,
                    bagMachineFilters: event?.filters,
                  },
                }),
              );
            }}
          ...
          <CustomPaginator
            dataList={bagMachineList}
            pageLimit={pageLimit}
            onPageChange={onPageChange}
            onPageRowsChange={onPageRowsChange}
            currentPage={currentPage}
            totalCount={bagMachineCount}
          />
        </div>
        <Dialog
          header={`${selectedBagMachine?._id ? 'Edit' : 'Add'} Bag Machine`}
          ...
          onHide={() => {
            ...
            resetForm();
            dispatch(
              setSelectedBagMachine({ name: '', code: '', is_active: 1 }),
            );
          }}
        >
          ...
          <div className="button_group d-flex align-items-center justify-content-end pt-3">
            ...
            >
              {selectedBagMachine?._id ? 'Update' : 'Save'}
            </Button>
          </div>
        </Dialog>
      </div>
