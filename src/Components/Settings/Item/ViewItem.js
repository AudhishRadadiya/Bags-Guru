...
function ViewItem() {
  ...
  // const { isGetInitialValuesItem, viewSelectedItemData } = useSelector(
  //   ({ rawitem }) => rawitem,
  // );
  ...
  const handleSalesItems = async () => {
    ...
    if (res) {
      ...
      // dispatch(setViewSelectedItemData(selectedRawItem));
    }
  };

  useEffect(() => {
    if (item_id) {
      // if (isGetInitialValuesItem?.view === true) {
      //   setInitialData(viewSelectedItemData);
      // } else {
      handleSalesItems();
      // }
    }
