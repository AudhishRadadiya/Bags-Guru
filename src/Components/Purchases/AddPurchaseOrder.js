...
export default function AddPurchaseOrder() {
  ...
  const partitionList = useCallback(() => {
    let FinalArray = [];
    if (colorGsmList?.length > 0) {
      let data = colorGsmList?.map(d => {
  ...
  }, [colorGsmList, newItemData]);

  const submitHandle = useCallback(
    async values => {
      if (colorGsmList?.length > 0) {
        let data = partitionList();

        // Update gsm list for change the "total" key instead of "total_amount"
        const updatedgsmTableViewList = gsmTableViewList?.map(data => {
        ...
        if (purchase_id) {
          ...
          if (state?.isView) {
            const itemList = gsmTableViewList?.map(d => {
  ...
  const handleAddUpdate = () => {
    ...
    const newItemDataKeys = Object.keys(newItemData)?.reduce((acc, key) => {
      const modifiedKey = removeSpecialCharacter(key);
      acc[modifiedKey] = newItemData[key];
      return acc;
    }, {});
    ...
    if (checkErrorField?.length > 0) {
      ...
      if (colorGsmList?.length > 0) {
          ...
          let gsmObj = i1?.gsmList.map(item => {
            ...
            Object.entries(filterColumn).forEach((d, i) => {
              if (d[1] && item['shaft'] && item['rate'] && item['gsm']) {
                ...
                let amount =
                  newItemData?.rate_in?.toLowerCase() === 'kg'
        ...
        const amount_calculation =
          newItemData?.rate_in?.toLowerCase() === 'kg'
            ? newItemData?.qty * newItemData?.rate * newItemData?.net_weight
            : newItemData?.qty * newItemData?.rate;

        if (
          selectedItem?.name?.toLowerCase() === 'nonwoven laminated pre-printed'
        ) {
          let data = [];
          ...
          let newWhiteCylinderAmount =
            newItemData?.new_white_cylinder_qty *
            newItemData?.new_white_cylinder_rate;
          ...
          let newColouredCylinderAmount =
            newItemData?.new_coloured_cylinder_qty *
            newItemData?.new_coloured_cylinder_rate;
          ...
          let reEngravedCylinderAmount =
            newItemData?.reengraved_cylinder_qty *
            newItemData?.reengraved_cylinder_rate;
          ...
          if (newItemDataKeys?.id) {
            let updatedList = [...prePrintedTableViewList];
            ...
            const allItemData = [...prePrintedTableViewList, obj];
            data = allItemData;
          }
          ...
          let amount =
            newItemData?.rate_in?.toLowerCase() === 'kg'
              ? newItemData?.qty *
                newItemData?.rate *
                (newItemData?.net_weight ? newItemData?.net_weight : 1)
              : newItemData?.qty * newItemData?.rate;
          ...
          if (newItemDataKeys?.id) {
            ...
            const allItemData = [...itemData, obj];
            data = allItemData;
          }
  ...
  const filteredBillingList = useMemo(() => {
    ...
    return Object.keys(foundItem)?.length > 0
      ...
      : [...filteredList];
  }, [billingList, values?.bill_to_address]);
  ...
  const filteredShippingList = useMemo(() => {
    ...
    return Object.keys(foundItem)?.length > 0
      ? [...filteredList, { ...foundItem }]
      : [...filteredList];
  }, [shippingList, values?.ship_to_address]);
  ...
  const onCustomChange = useCallback(
    (key, val) => {
      ...
      } else list = [...values?.ship_to_address];
      list[0] = companyAddress?.company_address?.find(x => x?._id === val);
  ...
  const handleAddgsm = color => {
    let colorGsmListData = [...colorGsmList];
  ...
  const renderNonWovenNonLaminatedList = useMemo(() => {
    const firstDataObject = getObjectWithMaxLength(gsmTableViewList);
    ...
    return Object.keys(data).map(key => ({
      field: key,
      header: key.charAt(0).toUpperCase() + key.slice(1).replace('_', ' '), // Capitalize the first letter
  ...
  }, [gsmTableViewList, rawItemListById?.item_attribute]);

  const renderItemList = useMemo(() => {
    const firstDataObject = getObjectWithMaxLength(itemData);
    let data = _.omit(firstDataObject, [
      'id',
      'total',
      'material',
  ...
  }, [itemData, rawItemListById?.item_attribute]);

  const renderPrePrintedItemList = useMemo(() => {
    return Object.keys(prePrintedTableHeader).map(key => ({
      field: key,
      header: key.charAt(0).toUpperCase() + key.slice(1)?.replaceAll('_', ' '), // Capitalize the first letter
      body: rawItemListById?.item_attribute?.find(
        item => item?.type === 6 && formatString(item?.name) === key,
  ...
  }, [rawItemListById?.item_attribute, prePrintedTableHeader]);

  const handleDeleteItemTableView = (data, index) => {
    let itemViewListData = [...itemData];
  ...
  const handleDeletePrePrintedItemTableView = (data, index) => {
    let itemViewListData = [...prePrintedTableViewList];
    let filteredData = itemViewListData?.filter((d, i) => data?.id !== d?.id);
    let updatedList = jobForPOList?.map(d => {
      if (d?._id === data?._id) {
    ...
    let filteredSelectedData = selectedJobList?.filter(
      (d, i) => data?._id !== d?._id,
    );
    let totalOfAmount = totalCount(filteredData, 'amount');
    let TotalTax = totalCount(filteredData, 'tax_amount');
  ...
  const handleSearchInput = e => {
    dispatch(getJobForPurchaseOrderList(e.target.value));
  };
    ...
    <div className="main_Wrapper">
      ...
      <div className="add_purchase_order_wrap">
                  ...
                  <div className="form_group mb-3">
                    ...
                    <ReactSelectSingle
                      ...
                      onChange={e => {
                        ...
                        dispatch(getActiveRawItemListByGroup(e.value));
                      }}
                      onBlur={handleBlur}
                  ...
                  <div className="form_group mb-3">
                    ...
                    <ReactSelectSingle
                      ...
                      onChange={async e => {
                        ...
                        const findedData = activeRawItemListByGroup?.find(
                          i => i?._id?.toLowerCase() === e.value,
                        );

                        // For set Table Header of Pre-Printed Table (From-Job).
                        if (findedData) setSelectedItem(findedData);
        ...
        <div className="table_main_Wrapper bg-white">
                        ...
                        {selectedItem?.name?.toLowerCase() ===
                          ...
                          <li>
                            <Button
                              ...
                              onClick={e => {
                                if (jobForPOList?.length === 0) {
                                  dispatch(getJobForPurchaseOrderList());
                                }
          ...
          <div className="data_table_wrapper cell_padding_large vertical_top break_header without_all_select ">
            {gsmTableViewList?.length > 0 && (
              <DataTable
                value={gsmTableViewList}
                sortMode="single"
                sortField="name"
                sortOrder={1}
                rows={10}
              ...
              >
                ...
                {renderNonWovenNonLaminatedList.map(col => (
                  <Column
                    key={col.field}
                    field={col.field}
                    header={col.header}
                ...
                <ul>
                  <li>
                    <Row className="align-items-center mb-3">
                      ...
                      <Col sm={8}>
                        <span>
                          {/* {parseFloat(Number(values?.sub_total))?.toFixed(2)} */}
                          {roundValueThousandSeparator(
                            values?.sub_total,
                            'decimal',
                          )}
                  ...
                  <li>
                    <Row className="align-items-center mb-3">
                      ...
                      <Col sm={8}>
                        <span>
                          {/* {parseFloat(Number(values?.tax))?.toFixed(2)} */}
                          {roundValueThousandSeparator(values?.tax, 'decimal')}
                        </span>
                      </Col>
                    </Row>
                  </li>
                  ...
                  <li>
                    <Row className="align-items-center mb-3">
                      ...
                      <Col sm={8}>
                        <span>
                          {/* {parseFloat(Number(values?.total))?.toFixed(2)} */}
                          {roundValueThousandSeparator(
                            values?.total,
                            'decimal',
                          )}
      ...
      <Dialog
        ...
        onHide={() => {
          ...
          const result = areObjectsEqual(newItemData, dummyNewItemData);
          if (!result) {
      ...
      >
        {selectedItem?.name?.toLowerCase() === 'nonwoven non laminated' && (
        ...
        <div className="add_product_content_wrap">
                  ...
                  {data?.type === 1 ? (
                    <div className="form_group col-md-3 mb-3">
                      ...
                      <InputText
                        ...
                        value={newItemData[formatString(data?.name)]}
                        onChange={e => handleItemChange(e, e.target.value)}
                      />
                      {isValidate &&
                        data?.is_mandatory === true &&
                        !newItemData[formatString(data?.name)] &&
                    ...
                    <div className="form_group col-md-3 mb-3">
                            ...
                            if (
                              ...
                              if (colorGsmList?.length > 0) {
                                ...
                                if (data?.length === 0) {
                                  ...
                                  let updatedList = colorGsmList?.filter(
                                    i => i?.colorName !== data[0]?.colorName,
                                  );
                      ...
                      {isValidate &&
                        data?.is_mandatory === true &&
                        !newItemData[formatString(data?.name)]?.length &&
                        errorRender(data?.name)}
                    </div>
                  ) : data?.type === 4 ? (
                    ...
                    <div className="form_group mb-3 col-md-3">
                      <Checkbox
                        ...
                        name={formatString(data?.name)}
                        value={newItemData[formatString(data?.name)]}
                        checked={newItemData[formatString(data?.name)]}
                        disabled={state?.isView}
                      ></Checkbox>
          ...
          {selectedItem?.name?.toLowerCase() === 'nonwoven non laminated' && (
            <div className="fabric_order_main_wrapper">
              {values?.item === selectedItem?._id &&
              (newItemData['color']?.length > 0 ||
                newItemData['fabric_color']?.length > 0) &&
              colorGsmList?.length > 0 ? (
                colorGsmList?.map(item => {
                  return (
                    <>
          ...
          <div className="mt-3 d-flex justify-content-end">
            <Button
              ...
              onClick={() => {
                ...
                if (colorGsmList?.length > 0) {
                  if (
                    !areArraysOfObjectsEqual(colorGsmList, dummyColorGsmList)
                  ) {
                    setColorGsmList(dummyColorGsmList);
                  }
                }
        ...
        <div className="add_product_content_wrap">
          ...
          <div className="button_group d-flex justify-content-end mt-3">
            ...
            <Button
              className="btn_primary ms-3"
              onClick={() => {
                const updatedJobForPOList = jobForPOList?.filter(item => {
                  return selectedJobList.some(
